...
void SimpleTweakingInterface::ThrowIfInvalidTweakLength(size_t length)
{
	if (!IsValidTweakLength(length))
		throw InvalidTweakLength(GetAlgorithm().AlgorithmName(), length);
}
...
void TweakableBlockCipher::ProcessAndXorBlock(const byte *inBlock, const byte *xorBlock, byte *outBlock) const
{
	const size_t FakeTweakLength = DefaultTweakLength();
	SecByteBlock FakeTweak(FakeTweakLength);
	memset_z(FakeTweak,0,FakeTweakLength);
	ProcessAndXorBlockWithTweak(inBlock,xorBlock,outBlock,FakeTweak,FakeTweakLength);
}
