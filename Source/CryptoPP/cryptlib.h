...
public:
	...
	//! returns the smallest valid tweak length in bytes that is >= min(n, GetMaxTweakLength())
...
//! interface for one direction (encryption or decryption) of a tweakable block cipher
/*! \note These objects usually should not be used directly. See BlockTransformation for more details. */
class CRYPTOPP_DLL CRYPTOPP_NO_VTABLE TweakableBlockCipher : public BlockCipher, public SimpleTweakingInterface
{
public:
	//! encrypt or decrypt inBlock using Tweak of length TweakLength, xor with xorBlock and write to outBlock
	virtual void ProcessAndXorBlockWithTweak(const byte *inBlock, const byte *xorBlock, byte *outBlock,const byte* Tweak,const size_t TweakLength) const =0;

	//! encrypt or decrypt inBlock, xor with xorBlock, and write to outBlock
	virtual void ProcessAndXorBlock(const byte *inBlock, const byte *xorBlock, byte *outBlock) const;
protected:
	const Algorithm & GetAlgorithm() const {return *this;}
};
