...
//! _
class CRYPTOPP_DLL InvalidTweakLength : public InvalidArgument
{
public:
	explicit InvalidTweakLength(const std::string &algorithm, size_t length) : InvalidArgument(algorithm + ": " + IntToString(length) + " is not a valid tweak length") {}
};
...
//! _
class CRYPTOPP_DLL InvalidState : public Exception
{
public:
	explicit InvalidState(const std::string &algorithm) : Exception(OTHER_ERROR,algorithm + ": State does not permit the requested operation.") {}
};
