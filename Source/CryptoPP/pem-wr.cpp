...
static void PEM_CipherForAlgorithm(string algorithm, auto_ptr<StreamTransformation>& stream,
...
static void PEM_SavePublicKey(BufferedTransformation& bt, const PUBLIC_KEY& key, const SecByteBlock& pre, const SecByteBlock& post);

template <class PRIVATE_KEY>
static void PEM_SavePrivateKey(BufferedTransformation& bt, const PRIVATE_KEY& key, const SecByteBlock& pre, const SecByteBlock& post);

template <class KEY>
static void PEM_SaveKey(BufferedTransformation& bt, const KEY& key, const SecByteBlock& pre, const SecByteBlock& post);
...
{
    PEM_SavePrivateKey(bt, rsa, SBB_RSA_PRIVATE_BEGIN, SBB_RSA_PRIVATE_END, algorithm, password, length);
}
...
void PEM_Save(BufferedTransformation& bt, const DSA::PrivateKey& dsa, const string& algorithm, const char* password, size_t length)
{
    PEM_SavePrivateKey(bt, dsa, SBB_DSA_PRIVATE_BEGIN, SBB_DSA_PRIVATE_END, algorithm, password, length);
}
...
void PEM_Save(BufferedTransformation& bt, const DL_PrivateKey_EC<ECP>& ec, const std::string& algorithm, const char* password, size_t length)
{
    bool old = PEM_GetNamedCurve(ec.GetGroupParameters());
    PEM_SetNamedCurve(ec.GetGroupParameters(), true);
    
    PEM_SavePrivateKey(bt, ec, SBB_EC_PRIVATE_BEGIN, SBB_EC_PRIVATE_END, algorithm, password, length);
    PEM_SetNamedCurve(ec.GetGroupParameters(), old);
}
...
void PEM_Save(BufferedTransformation& bt, const DL_PrivateKey_EC<EC2N>& ec, const std::string& algorithm, const char* password, size_t length)
{
    bool old = PEM_GetNamedCurve(ec.GetGroupParameters());
    PEM_SetNamedCurve(ec.GetGroupParameters(), true);
...
void PEM_Save(BufferedTransformation& bt, DL_Keys_ECDSA<ECP>::PrivateKey& ecdsa, const std::string& algorithm, const char* password, size_t length)
{
    PEM_Save(bt, dynamic_cast<DL_PrivateKey_EC<ECP>&>(ecdsa), algorithm, password, length);
}
...
template <class PUBLIC_KEY>
void PEM_SavePublicKey(BufferedTransformation& bt, const PUBLIC_KEY& key, const SecByteBlock& pre, const SecByteBlock& post)
{
                         ...
                         const std::string& algorithm, const char* password, size_t length)
{
    ...
    PEM_CipherForAlgorithm(algorithm, stream, _key, _iv, password, length);
...
void PEM_CipherForAlgorithm(string algorithm, auto_ptr<StreamTransformation>& stream,
                            SecByteBlock& key, SecByteBlock& iv, const char* password, size_t length)
{
    ...
    AutoSeededRandomPool& prng = GetRandomPool();
    prng.GenerateBlock(_iv.data(), _iv.size());
    _salt = _iv;
