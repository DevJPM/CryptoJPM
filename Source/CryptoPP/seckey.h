...
public:
	...
	static size_t CRYPTOPP_API StaticGetValidTweakLength(size_t) {return TWEAKLENGTH;}
};

/// support query of variable tweak length, template parameters are default, min, max, multiple (default multiple 1)
...
class CRYPTOPP_NO_VTABLE SimpleKeyingInterfaceImpl : public virtual BASE
{
public:
...
//! _
template <class BASE, class INFO = BASE>
class CRYPTOPP_NO_VTABLE SimpleTweakingInterfaceImpl : public virtual BASE
{
public:
	size_t MinTweakLength() const {return INFO::MIN_TWEAKLENGTH;}
	size_t MaxTweakLength() const {return (size_t)INFO::MAX_TWEAKLENGTH;}
	size_t DefaultTweakLength() const {return INFO::DEFAULT_TWEAKLENGTH;}
	size_t GetValidTweakLength(size_t n) const {return INFO::StaticGetValidTweakLength(n);}
};

//! _
template<class BASE, class INFO = BASE>
class CRYPTOPP_NO_VTABLE SimpleTweakableKeyingInterfaceImpl : public SimpleTweakingInterfaceImpl<BASE,INFO>, public SimpleKeyingInterfaceImpl<BASE,INFO>
{
};
...
template <class INFO, class BASE = TweakableBlockCipher>
class CRYPTOPP_NO_VTABLE TweakableBlockCipherImpl : public AlgorithmImpl<SimpleTweakableKeyingInterfaceImpl<TwoBases<BASE, INFO> > >
{
public:
	unsigned int BlockSize() const {return this->BLOCKSIZE;}
};
...
//! _
template <CipherDir DIR, class BASE>
class TweakableBlockCipherFinal : public ClonableImpl<TweakableBlockCipherFinal<DIR, BASE>, BASE>
{
public:
 	TweakableBlockCipherFinal() {}
	TweakableBlockCipherFinal(const byte *key)
		{this->SetKey(key, this->DEFAULT_KEYLENGTH);}
	TweakableBlockCipherFinal(const byte *key, size_t length)
		{this->SetKey(key, length);}
	TweakableBlockCipherFinal(const byte *key, size_t length, unsigned int rounds)
		{this->SetKeyWithRounds(key, length, rounds);}
